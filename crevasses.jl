...
function fb_dvv(ξ, Tb,Ts, H)
	dz = 1e-2;
	zz = dz:dz:(1-dz);
	BB = B.(zz, Tb*ones(size(zz)), Ts*ones(size(zz)),H*ones(size(zz)));
	BS = B(H,Tb,Ts,H);
	G = get_G(zz,ξ)
	integrand = BB/BS .* G .* (1-ξ)^(-3/2) .* (1 .- zz.^2).^(-1/2);
	return sum(integrand)*dz
end
...
begin
	...
	Ki_base_dvv = KIi_base(dbb,h) + KIw_base(dbb,h) +  KIB(dbb,h,strain_rate, Tbb, Tss)

	#stress distribution plot
	p1 = plot(dbb, Ki_base_dvv,xlim = [0,0.9*h], ylim = [-1,1]*1e6, label = "total", linewidth = 3,framestyle=:box, xlabel="basal crevasse depth")
	plot!(p1, dbb, KIi_base(dbb,h), label = "ice weight")
	plot!(p1, dbb, KIw_base(dbb,h), label = "water weight")
	plot!(p1, dbb,  KIB(dbb,h,strain_rate, Tbb, Tss), label = "tensile stress")
	plot!(dbb,zeros(size(dbb)),linecolor= :black, linestyle =:dash, label = :none)

	#find where the curve goes thru zero
	val,idx = findmin(abs.(Ki_base_dvv))
	cond = (val < 1e5) && (Ki_base_dvv[idx] > Ki_base_dvv[idx + 1])
	if cond
		scatter!([dbb[idx]], [0], label = :none, legend =:bottomleft)
	end
	

	#plot the shelf and the temperature gradient
	p2 = plot([-1,1], -0.9*[h,h],ylim = [-300,100],xlim = [-1,1] ,xticks = :none,linewidth = 2, linecolor = :black, label = :none, framestyle = :box)
	plot!(p2, [-1,1], 0.1*[h,h], linewidth = 2, linecolor = :black, label = :none)
	if cond
		plot!(p2, [0,0], [-0.9*h, (-0.9*h + dbb[idx])], linewidth = 2, linecolor = :black, label = :none)
	else
		plot!(p2, [0,0], [-0.9*h, 0.1*h], linewidth = 2, linecolor = :black, label = :none)
	end
	
	# plot the temperature gradient
	zz = range(0,stop=h,length=50)
	T = (Tss - Tbb) * (zz / h) .+ Tbb;
	p3 = plot(T,zz .- 0.9*h , label = :none, framestyle = :box, color = :red)
	plot!(p3,[-30,0], -0.9*[h,h],xlim = [-30,0],ylim = [-300,100] ,linewidth = 2, linecolor = :black, label = :none)
	plot!(p3, [-30,0], 0.1*[h,h], linewidth = 2, linecolor = :black, label = :none, xlabel = "temp (C)", ylabel = "depth (m)")
...
# ╔═╡ e3fa6ebc-8085-448c-93f6-f224f0ca149a
